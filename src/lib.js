...
function preprocessPattern(pattern, settings, margin = 0) {
  ...
  let smoothed = movingAverage(raw, effectiveSettings.smoothW);
  ...
  const normalized = normalizeSeries(x, corrected, effectiveSettings.normalizeMode);
...
export function processPatterns(patterns, settings) {
  ...
  return transformed.map((pattern, visibleIndex) => {
    ...
    const displayY = pattern.effectiveSettings?.normalizeMode === "none"
