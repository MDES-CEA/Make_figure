...
export function createWorkspace(mode = "drx", input = {}) {
  ...
  return {
    ...
    measurementCursors: input?.measurementCursors && typeof input.measurementCursors === "object"
      ? input.measurementCursors
      : { a: null, b: null },
  };
}
...
function validateWorkspace(input, mode) {
  ...
  const { settings, patterns, phases, notes, zones, measurementCursors } = workspace;
  ...
  const normalizeCursor = (cursor) => cursor && Number.isFinite(Number(cursor.x))
    ? { x: Number(cursor.x) }
    : null;
  return {
    settings,
    ...
    zones,
    measurementCursors: {
      a: normalizeCursor(measurementCursors?.a),
      b: normalizeCursor(measurementCursors?.b),
    },
  };
}
