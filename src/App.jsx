...
export default function App() {
  ...
  const { settings: S, patterns, phases, notes, zones = [], measurementCursors = { a: null, b: null } } = workspace;
  ...
  const [measureNext, setMeasureNext] = useState("a");
  ...
  const updateMeasurementCursor = useCallback((which, value) => {
    history.set((current) => updateWorkspaceProject(current, activeMode, (currentWorkspace) => ({
      ...currentWorkspace,
      measurementCursors: { ...(currentWorkspace.measurementCursors || { a: null, b: null }), [which]: value },
    })));
  }, [activeMode, history]);
  ...
  const saveSessionFile = useCallback(() => {
    const payload = JSON.stringify({ ...project, version: 11 }, null, 2);
  ...
  const measurementReadout = useMemo(() => {
    const read = (cursorValue) => cursorValue && activeProcessedPattern
      ? nearestValue(activeProcessedPattern, cursorValue.x)
      : null;
    const a = read(measurementCursors.a);
    const b = read(measurementCursors.b);
    return {
      a,
      b,
      dx: measurementCursors.a && measurementCursors.b ? measurementCursors.b.x - measurementCursors.a.x : null,
      dy: a && b ? b.y - a.y : null,
    };
  }, [activeProcessedPattern, measurementCursors.a, measurementCursors.b]);
  ...
  }, [H, M.left, M.top, S.xmax, S.xmin, W, mainHeight, plotWidth, yMaximum, yMinimum]);
  ...
  const onSvgPointerDown = (event) => {
    ...
    if (tool === "zoomRect") {
      interactionRef.current = { type: "zoom", pointerId: event.pointerId, start: point, payload: {} };
      svgRef.current?.setPointerCapture?.(event.pointerId);
      setZoomBox({ x1: point.svgX, y1: point.svgY, x2: point.svgX, y2: point.svgY });
    }
  };
  ...
  const onSvgPointerMove = (event) => {
    ...
    if (interaction) {
      if (interaction.type === "zoom") {
        setZoomBox({ x1: interaction.start.svgX, y1: interaction.start.svgY, x2: point.svgX, y2: point.svgY });
      } else if (interaction.type === "note") {
        setDragPreview({ type: "note", id: interaction.payload.id, x: clamp(point.dataX, S.xmin, S.xmax), yFrac: clamp(1 - ((point.svgY - M.top) / mainHeight), 0, 1) });
        ...
        setDragPreview({ type: "measure", which: interaction.payload.which, x: snapX(clamp(point.dataX, S.xmin, S.xmax)) });
  ...
  const finishSvgInteraction = (event) => {
    ...
    if (interaction.type === "zoom") {
      ...
      if (box && Math.abs(box.x2 - box.x1) > 8 && Math.abs(box.y2 - box.y1) > 8) {
        const left = Math.max(M.left, Math.min(box.x1, box.x2));
        const right = Math.min(M.left + plotWidth, Math.max(box.x1, box.x2));
        const top = Math.max(M.top, Math.min(box.y1, box.y2));
        const bottom = Math.min(M.top + mainHeight, Math.max(box.y1, box.y2));
        const xmin = S.xmin + ((left - M.left) / plotWidth) * (S.xmax - S.xmin);
        const xmax = S.xmin + ((right - M.left) / plotWidth) * (S.xmax - S.xmin);
        const viewYMax = yMaximum - ((top - M.top) / mainHeight) * (yMaximum - yMinimum);
        const viewYMin = yMaximum - ((bottom - M.top) / mainHeight) * (yMaximum - yMinimum);
        history.set((current) => updateWorkspaceProject(current, activeMode, (currentWorkspace) => ({
          ...currentWorkspace,
          settings: { ...currentWorkspace.settings, xmin, xmax, viewYMin, viewYMax },
        })));
      }
    } else if (dragPreview?.type === "note") {
      updateNote(dragPreview.id, "x", dragPreview.x);
      updateNote(dragPreview.id, "yFrac", dragPreview.yFrac);
    } else if (dragPreview?.type === "phaseLabel") {
      updatePhase(dragPreview.id, "labelOffsetX", dragPreview.xOffset);
      updatePhase(dragPreview.id, "labelOffsetY", dragPreview.yOffset);
    } else if (dragPreview?.type === "zoneBoundary") {
      updateZone(dragPreview.id, dragPreview.edge === "min" ? "xmin" : "xmax", dragPreview.x);
    } else if (dragPreview?.type === "measure") {
      updateMeasurementCursor(dragPreview.which, { x: dragPreview.x });
  ...
  const onSvgClick = (event) => {
    ...
    if (tool === "measure") {
      const x = snapX(clamp(point.dataX, S.xmin, S.xmax));
      updateMeasurementCursor(measureNext, { x });
      setMeasureNext((value) => value === "a" ? "b" : "a");
      return;
    }
        ...
        <section key={activeMode} className="canvas-column">
          <div className="canvas-toolbar">
            ...
            <div className="canvas-toolbar__group">
              ...
              <IconButton icon="zoomRect" title="Zoom rectangulaire" active={tool === "zoomRect"} onClick={() => setTool("zoomRect")} />
              <IconButton icon="ruler" title={`Curseurs de mesure · prochain : ${measureNext.toUpperCase()}`} active={tool === "measure"} onClick={() => setTool("measure")} />
            ...
            <span className="canvas-toolbar__hint">Rectangle : zoom X · double-clic : réinitialiser · labels et annotations : glisser</span>
          </div>
              ...
              <div className={`page-stage ${comparisonView ? "is-comparison" : ""}`} style={{ width: comparisonView ? W * displayZoom * 2 + 24 : W * displayZoom, height: H * displayZoom }}>
                {comparisonView && (
                  <div className="figure-page figure-page--raw" style={{ width: W * displayZoom, height: H * displayZoom }}>
                    <RawComparisonPreview data={rawProcessed} colors={colorMap} width={W} height={H} xmin={S.xmin} xmax={S.xmax} />
                  </div>
                )}
                <div className="figure-page" style={{ width: W * displayZoom, height: H * displayZoom }}>
                  ...
                  >
                    ...
                    {S.mode === "raman" && zones.filter((zone) => zone.visible && Number(zone.xmax) > S.xmin && Number(zone.xmin) < S.xmax).map((zone) => {
                      ...
                      const start = Math.max(S.xmin, previewMin);
                      const end = Math.min(S.xmax, previewMax);
                      const x = xToPx(start);
                    ...
                    {processed.map((pattern) => {
                      ...
                      return (
                        <g key={pattern.id} opacity={selectedByType.pattern.size && !isSelected("pattern", pattern.id) ? 0.72 : 1}>
                          ...
                          <text
                            x={xToPx(S.xmax) + 10}
                            y={dragPreview?.type === "curveOrder" && dragPreview.id === pattern.id ? dragPreview.svgY : yToPx(labelY)}
                            dominantBaseline="middle"
                            fontSize={S.patternLabelSize}
                            fontWeight={S.patternLabelBold ? "700" : "400"}
                            fill={color}
                            fontFamily="Arial, Helvetica, sans-serif"
                            style={{ cursor: pattern.locked ? "not-allowed" : "ns-resize", userSelect: "none" }}
                            onClick={(event) => selectItem(event, "pattern", pattern.id)}
                            onPointerDown={(event) => { if (!pattern.locked) beginCanvasDrag(event, "curveOrder", { id: pattern.id }); }}
                          >{pattern.label}{pattern.isDifferenceReference ? " (réf.)" : ""}</text>
                        </g>
                      );
                    })}
                    ...
                    {notes.filter((note) => note.visible !== false).map((note) => {
                      const preview = dragPreview?.type === "note" && dragPreview.id === note.id ? dragPreview : note;
                      const x = xToPx(preview.x);
                      const y = M.top + mainHeight * (1 - preview.yFrac);
                      return (
                        <g
                          key={note.id}
                          opacity={isSelected("note", note.id) ? 1 : 0.92}
                          style={{ cursor: "move" }}
                          onClick={(event) => selectItem(event, "note", note.id)}
                          onPointerDown={(event) => beginCanvasDrag(event, "note", { id: note.id })}
                        >
                    ...
                    {(["a", "b"]).map((which) => {
                      const previewX = dragPreview?.type === "measure" && dragPreview.which === which ? dragPreview.x : measurementCursors?.[which]?.x;
                      if (!Number.isFinite(Number(previewX))) return null;
                      const x = xToPx(Number(previewX));
                      const value = activeProcessedPattern ? nearestValue(activeProcessedPattern, Number(previewX)) : null;
                      return <g key={`measure-${which}`} data-ui-only="true" style={{ cursor: "ew-resize" }} onPointerDown={(event) => beginCanvasDrag(event, "measure", { which })}>
                        <line x1={x} x2={x} y1={M.top} y2={M.top + mainHeight} stroke={which === "a" ? "#dc7848" : "#7c6cff"} strokeWidth="1.25" strokeDasharray="5 3" />
                        <rect x={x - 9} y={M.top + 3} width="18" height="16" rx="4" fill={which === "a" ? "#dc7848" : "#7c6cff"} />
                        <text x={x} y={M.top + 14} textAnchor="middle" fontSize="9" fontWeight="700" fill="#fff">{which.toUpperCase()}</text>
                        {value && <circle cx={xToPx(value.x)} cy={yToPx(value.y + (activeProcessedPattern?.stackOffset || 0))} r="4" fill="#fff" stroke={which === "a" ? "#dc7848" : "#7c6cff"} strokeWidth="1.4" />}
                      </g>;
                    })}

                    {zoomBox && <g data-ui-only="true" pointerEvents="none"><rect x={Math.min(zoomBox.x1, zoomBox.x2)} y={Math.min(zoomBox.y1, zoomBox.y2)} width={Math.abs(zoomBox.x2 - zoomBox.x1)} height={Math.abs(zoomBox.y2 - zoomBox.y1)} fill="#7c6cff" opacity="0.12" stroke="#7c6cff" strokeWidth="1.2" strokeDasharray="5 3" /></g>}
          ...
          <footer className="statusbar">
            ...
            {measurementCursors.a && <span>A = <strong>{measurementCursors.a.x.toFixed(S.mode === "drx" ? 3 : 1)}</strong></span>}
            {measurementCursors.b && <span>B = <strong>{measurementCursors.b.x.toFixed(S.mode === "drx" ? 3 : 1)}</strong></span>}
            {measurementReadout.dx !== null && <span className="statusbar__measurement">Δx = <strong>{measurementReadout.dx.toFixed(S.mode === "drx" ? 3 : 1)}</strong>{measurementReadout.dy !== null && <> · ΔI = <strong>{measurementReadout.dy.toFixed(4)}</strong></>}</span>}
              ...
              <>
                ...
                <Section title="Interaction avec la figure">
                  ...
                  <div className="measurement-card">
                    <span>Curseurs de mesure</span>
                    <strong>{measurementReadout.dx === null ? "Placer A et B" : `Δx ${measurementReadout.dx.toFixed(S.mode === "drx" ? 3 : 1)}${measurementReadout.dy === null ? "" : ` · ΔI ${measurementReadout.dy.toFixed(4)}`}`}</strong>
                    <div className="inline-actions"><Button variant="secondary" icon="ruler" onClick={() => setTool("measure")}>Placer {measureNext.toUpperCase()}</Button><Button variant="secondary" icon="trash" onClick={() => history.set((current) => updateWorkspaceProject(current, activeMode, (currentWorkspace) => ({ ...currentWorkspace, measurementCursors: { a: null, b: null } })))}>Effacer</Button></div>
                  </div>
                </Section>
