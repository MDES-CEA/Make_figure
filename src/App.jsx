...
export default function App() {
  ...
  const saveSessionFile = useCallback(() => {
    const payload = JSON.stringify({ ...project, version: 13 }, null, 2);
  ...
  const xToPx = useCallback((x) => M.left + ((x - viewXMin) / (viewXMax - viewXMin)) * plotWidth, [M.left, viewXMin, viewXMax, plotWidth]);
  const yToPx = useCallback((y) => M.top + mainHeight - ((y - yMinimum) / (yMaximum - yMinimum)) * mainHeight, [M.top, mainHeight, yMinimum, yMaximum]);
  const xTicks = useMemo(() => computeTicks(viewXMin, viewXMax, S.xTickStep), [viewXMin, viewXMax, S.xTickStep]);
  ...
  const annotationData = useMemo(() => {
    ...
    phases.forEach((phase) => {
      ...
      phase.peaks.forEach(([x, intensity]) => {
        if (x >= viewXMin && x <= viewXMax && intensity >= S.tickMinI) {
          ticks.push({
            x,
            intensity,
    ...
    return { ticks, labels };
  }, [S.showAnnotations, viewXMin, viewXMax, S.tickMinI, S.labelMinI, S.labelMinSep, phases]);
              ...
              <div className={`page-stage ${comparisonView ? "is-comparison" : ""}`} style={{ width: comparisonView ? W * displayZoom * 2 + 24 : W * displayZoom, height: H * displayZoom }}>
                ...
                <div className="figure-page" style={{ width: W * displayZoom, height: H * displayZoom }}>
                  ...
                  >
                    ...
                    {S.mode === "raman" && zones.filter((zone) => zone.visible && Number(zone.xmax) > viewXMin && Number(zone.xmin) < viewXMax).map((zone) => {
                    ...
                    {panelHeight > 0 && (
                      <g>
                        {panelPhases.map((phase, rowIndex) => {
                          ...
                          return (
                            <g key={phase.id}>
                              {phase.peaks.map(([x, intensity], index) => x >= viewXMin && x <= viewXMax ? (
