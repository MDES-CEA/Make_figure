...
function NumberField({ label, value, onChange, min, max, step = 1, suffix, compact = false, targetId }) {
  const commit = (event) => {
...
function RangeNavigator({ patterns, fullRange, xmin, xmax, onPreview, onCommit, onCancel }) {
  ...
  }, [xmin, xmax, fullRange.minimum, fullRange.maximum]);
  ...
  const start = (event, action) => {
    ...
    dragRef.current = {
      ...
      startMin: Number(xmin),
      startMax: Number(xmax),
  ...
  const move = (event) => {
    ...
    if (!drag) return;
    const minimumSpan = Math.max(width * 0.002, 1e-9);
    const delta = ((event.clientX - drag.startClientX) / Math.max(1, drag.rectWidth)) * width;
    let nextMin = drag.startMin;
    let nextMax = drag.startMax;

    if (drag.action === "left") {
      nextMin = clamp(drag.startMin + delta, fullRange.minimum, drag.startMax - minimumSpan);
    } else if (drag.action === "right") {
      nextMax = clamp(drag.startMax + delta, drag.startMin + minimumSpan, fullRange.maximum);
    } else {
      const span = drag.startMax - drag.startMin;
      nextMin = drag.startMin + delta;
      nextMax = drag.startMax + delta;
      if (nextMin < fullRange.minimum) {
        nextMin = fullRange.minimum;
        nextMax = fullRange.minimum + span;
      }
      if (nextMax > fullRange.maximum) {
        nextMax = fullRange.maximum;
        nextMin = fullRange.maximum - span;
      }
    }

    const next = { xmin: nextMin, xmax: nextMax };
    draftRef.current = next;
    setDraft(next);
    onPreview?.(nextMin, nextMax, drag.action);
  };
  ...
  const leftPct = clamp(toPct(view.xmin), 0, 100);
  const rightPct = clamp(toPct(view.xmax), 0, 100);
  return (
    <div
    ...
    <div className={`app-shell mode-${activeMode} density-${uiDensity} ${reduceMotion ? "reduce-motion" : ""} ${editorFullscreen ? "is-editor-fullscreen" : ""}`}>
      <header className="topbar masthead">
        <div className="masthead__edition">
          ...
          <span>Vol. 16 · Interaction desk · Browser-local processing</span>
      ...
      <main className="workbench" style={{ gridTemplateColumns: `${leftCollapsed ? 0 : leftWidth}px minmax(300px, 1fr) ${rightCollapsed ? 0 : rightWidth}px` }}>
        ...
        <section key={activeMode} className="canvas-column">
          <div className="canvas-toolbar">
            ...
            <span className="canvas-toolbar__hint">Navigateur de plage : poignée gauche = X min · poignée droite = X max · fenêtre centrale = déplacement · double-clic sur l’axe : réinitialiser</span>
          </div>
