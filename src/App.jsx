...
export default function App() {
  ...
  const saveSessionFile = useCallback(() => {
    const payload = JSON.stringify({ ...project, version: 12 }, null, 2);
      ...
      <main className="workbench" style={{ gridTemplateColumns: `${leftCollapsed ? 0 : leftWidth}px minmax(300px, 1fr) ${rightCollapsed ? 0 : rightWidth}px` }}>
        ...
        <section key={activeMode} className="canvas-column">
          <div className="canvas-toolbar">
            ...
            <span className="canvas-toolbar__hint">Axe X : glisser la moitié gauche pour X min, la moitié droite pour X max · double-clic : réinitialiser · labels et notes : glisser</span>
          </div>
                ...
                <div className="figure-page" style={{ width: W * displayZoom, height: H * displayZoom }}>
                  ...
                  >
                      ...
                      return (
                        <g key={pattern.id} opacity={selectedByType.pattern.size && !isSelected("pattern", pattern.id) ? 0.72 : 1}>
                          ...
                          {(() => {
                            ...
                            const labelX = xToPx(S.xmax) + 10 + dx;
                    ...
                    {notes.filter((note) => note.visible !== false).map((note) => {
                      const moving = dragPreview?.type === "note" && dragPreview.id === note.id ? dragPreview : null;
                      const resizing = dragPreview?.type === "noteResize" && dragPreview.id === note.id ? dragPreview : null;
                      const x = xToPx(moving ? moving.x : note.x);
                      const yFrac = moving ? moving.yFrac : note.yFrac;
                      const y = M.top + mainHeight * (1 - yFrac);
                      const fontSize = resizing ? resizing.fontSize : Number(note.fontSize) || 10;
                      const selected = isSelected("note", note.id);
                      const estimatedWidth = Math.max(24, String(note.text || "").length * fontSize * 0.58);
                      return (
                        <g key={note.id} opacity={selected ? 1 : 0.92}>
                          ...
                          <text
                            x={x} y={y} textAnchor="middle" fontSize={fontSize} fill={note.color} fontFamily="Arial, Helvetica, sans-serif"
                            transform={note.rotation ? `rotate(${note.rotation} ${x} ${y})` : undefined}
                            style={{ cursor: "move", userSelect: "none" }}
                            onClick={(event) => selectItem(event, "note", note.id)}
                            onPointerDown={(event) => beginCanvasDrag(event, "note", { id: note.id, x: note.x, yFrac: note.yFrac, fontSize })}
                          >{note.text}</text>
                          {selected && <g data-ui-only="true">
                            <rect x={x - estimatedWidth / 2 - 4} y={y - fontSize - 4} width={estimatedWidth + 8} height={fontSize + 10} fill="none" stroke={note.color} strokeWidth="0.8" strokeDasharray="3 2" opacity="0.7" pointerEvents="none" />
                            <rect x={x + estimatedWidth / 2 + 1} y={y - 5} width="8" height="8" rx="2" fill={note.color} stroke="#fff" strokeWidth="1" style={{ cursor: "nwse-resize" }} onPointerDown={(event) => beginCanvasDrag(event, "noteResize", { id: note.id, fontSize })} />
                          </g>}
                        </g>
                      );
                    })}
                    ...
                    {(() => {
                      ...
                      const previewMin = dragPreview?.type === "xAxisRange" ? dragPreview.xmin : S.xmin;
                      const previewMax = dragPreview?.type === "xAxisRange" ? dragPreview.xmax : S.xmax;
                      return <g>
                        <line x1={M.left} x2={M.left + plotWidth} y1={axisY} y2={axisY} stroke="#15191f" strokeWidth="1"/>
                        {xTicks.map((tick) => <g key={tick}><line x1={xToPx(tick)} x2={xToPx(tick)} y1={axisY} y2={axisY + 5} stroke="#15191f" strokeWidth="1"/><text x={xToPx(tick)} y={axisY + 20} textAnchor="middle" fontSize={S.tickFontSize} fill="#15191f" fontFamily="Arial, Helvetica, sans-serif">{tick}</text></g>)}
                        <text x={M.left + plotWidth / 2} y={axisY + 42} textAnchor="middle" fontSize={S.axisFontSize} fill="#15191f" fontFamily="Arial, Helvetica, sans-serif">{S.xlabel}</text>
