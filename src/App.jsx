...
function RangeNavigator({ patterns, fullRange, xmin, xmax, onCommit }) {
  const ref = useRef(null);
  const dragRef = useRef(null);
  const [draft, setDraft] = useState(null);
  const view = draft || { xmin, xmax };
  const width = Math.max(1e-12, fullRange.maximum - fullRange.minimum);
  const toPct = (value) => ((value - fullRange.minimum) / width) * 100;
  const fromClient = (clientX) => {
    const rect = ref.current?.getBoundingClientRect();
    if (!rect) return fullRange.minimum;
    return fullRange.minimum + clamp((clientX - rect.left) / rect.width, 0, 1) * width;
  };
  const start = (event, action) => {
    event.preventDefault();
    const value = fromClient(event.clientX);
    dragRef.current = { action, start: value, xmin: view.xmin, xmax: view.xmax, pointerId: event.pointerId };
  ...
  const move = (event) => {
    ...
    if (!drag) return;
    const value = fromClient(event.clientX);
    const minimumSpan = width * 0.002;
    if (drag.action === "left") setDraft({ xmin: Math.min(value, view.xmax - minimumSpan), xmax: view.xmax });
    else if (drag.action === "right") setDraft({ xmin: view.xmin, xmax: Math.max(value, view.xmin + minimumSpan) });
    else {
      const span = drag.xmax - drag.xmin;
      const delta = value - drag.start;
      let nextMin = drag.xmin + delta;
      let nextMax = drag.xmax + delta;
      if (nextMin < fullRange.minimum) { nextMax += fullRange.minimum - nextMin; nextMin = fullRange.minimum; }
      if (nextMax > fullRange.maximum) { nextMin -= nextMax - fullRange.maximum; nextMax = fullRange.maximum; }
      setDraft({ xmin: nextMin, xmax: nextMax });
    }
  };
  const finish = () => {
    if (draft) onCommit(draft.xmin, draft.xmax);
  ...
  const overview = patterns.filter((pattern) => pattern.visible !== false).slice(0, 8).map((pattern) => {
    ...
    for (let i = 0; i < pattern.x.length; i += stride) {
      const x = toPct(pattern.x[i] + (Number(pattern.xoffset) || 0));
  ...
  return (
    <div className="range-navigator" ref={ref} onPointerMove={move} onPointerUp={finish} onPointerCancel={finish}>
      <svg viewBox="0 0 100 42" preserveAspectRatio="none" aria-label="Navigateur de plage X">
        ...
        <rect x={toPct(view.xmin)} y="1" width={Math.max(0.5, toPct(view.xmax) - toPct(view.xmin))} height="40" className="range-navigator__selection" onPointerDown={(event) => start(event, "move")} />
        <line x1={toPct(view.xmin)} x2={toPct(view.xmin)} y1="1" y2="41" className="range-navigator__handle" onPointerDown={(event) => start(event, "left")} />
        <line x1={toPct(view.xmax)} x2={toPct(view.xmax)} y1="1" y2="41" className="range-navigator__handle" onPointerDown={(event) => start(event, "right")} />
      </svg>
      <span>{fullRange.minimum.toFixed(1)}</span><strong>{view.xmin.toFixed(1)} — {view.xmax.toFixed(1)}</strong><span>{fullRange.maximum.toFixed(1)}</span>
    </div>
  );
}
...
export default function App() {
  ...
  const axisPreviewActive = dragPreview?.type === "xAxisRange"
  ...
  const saveSessionFile = useCallback(() => {
    const payload = JSON.stringify({ ...project, version: 14 }, null, 2);
  ...
  const alignVisiblePatterns = () => {
    ...
    history.set((current) => updateWorkspaceProject(current, activeMode, (currentWorkspace) => ({
      ...
      patterns: currentWorkspace.patterns.map((pattern) => {
        ...
        if (!result || pattern.locked) return pattern;
        return {
          ...pattern,
  ...
  const M = { left: S.figureLayoutMode === "single" ? 62 : 22, right: S.figureLayoutMode === "single" ? S.rightMargin : 22, top: S.title ? 48 : 22, gap: 10, axisHeight: S.figureLayoutMode === "single" ? 50 : 10 };
  ...
  const axisCoordinate = useCallback((x) => drxAxisMode === "native" ? Number(x) : convertDrxX(x, drxAxisMode, Number(S.wavelength) || 1.5406), [drxAxisMode, S.wavelength]);
  ...
  const xToPx = useCallback((x) => {
    ...
    const start = axisCoordinate(viewXMin); const end = axisCoordinate(viewXMax); const current = axisCoordinate(x);
    return M.left + ((current - start) / Math.max(1e-12, end - start)) * plotWidth;
  }, [M.left, S.brokenAxisEnd, S.brokenAxisGapPx, S.brokenAxisStart, axisCoordinate, breakActive, plotWidth, viewXMax, viewXMin]);
  const pxToDataX = useCallback((px) => {
    ...
    const axisStart = axisCoordinate(viewXMin);
    const axisEnd = axisCoordinate(viewXMax);
    const coordinate = axisStart + fraction * (axisEnd - axisStart);
    if (activeMode !== "drx" || drxAxisMode === "native" || drxAxisMode === "2theta") return coordinate;
    return invertDrxX(coordinate, drxAxisMode, Number(S.wavelength) || 1.5406);
  }, [M.left, S.brokenAxisEnd, S.brokenAxisGapPx, S.brokenAxisStart, S.wavelength, activeMode, axisCoordinate, breakActive, drxAxisMode, plotWidth, viewXMax, viewXMin]);
  ...
  const xTickObjects = useMemo(() => {
    if (activeMode !== "drx" || drxAxisMode === "2theta") return computeTicks(viewXMin, viewXMax, S.xTickStep).filter((tick) => !breakActive || tick <= Number(S.brokenAxisStart) || tick >= Number(S.brokenAxisEnd)).map((tick) => ({ x: tick, label: String(tick) }));
    const a = axisCoordinate(viewXMin); const b = axisCoordinate(viewXMax); const low = Math.min(a, b); const high = Math.max(a, b);
    return computeTicks(low, high, 0).map((value) => ({ x: invertDrxX(value, drxAxisMode, Number(S.wavelength) || 1.5406), label: drxAxisMode === "d" ? value.toFixed(2) : value.toFixed(2) })).filter((tick) => Number.isFinite(tick.x) && tick.x >= viewXMin && tick.x <= viewXMax).sort((left, right) => left.x - right.x);
  }, [S.brokenAxisEnd, S.brokenAxisStart, S.wavelength, S.xTickStep, activeMode, axisCoordinate, breakActive, drxAxisMode, viewXMax, viewXMin]);
  ...
  const addLibraryPhase = useCallback((entry) => {
    const phase = { ...entry, id: newId("phase"), libraryKey: undefined, savedAt: undefined, files: [...(entry.files || []), "bibliothèque locale"] };
    history.set((current) => updateWorkspaceProject(current, "drx", (currentWorkspace) => ({ ...currentWorkspace, phases: [...currentWorkspace.phases, phase] })));
    setMessage(`Phase « ${phase.name} » ajoutée depuis la bibliothèque.`);
  }, [history]);
  ...
  const beginCanvasDrag = useCallback((event, type, payload = {}) => {
    ...
    interactionRef.current = { type, pointerId: event.pointerId, start: point, payload };
    setDragPreview({ type, ...payload });
  }, [svgPoint]);
  ...
  const onSvgPointerMove = (event) => {
    ...
    if (interaction) {
      ...
      } else if (interaction.type === "xAxisRange") {
        const minimumSpan = Math.max((fullXRange.maximum - fullXRange.minimum) * 0.001, 1e-9);
        const x = clamp(point.dataX, fullXRange.minimum, fullXRange.maximum);
        setDragPreview(interaction.payload.edge === "min"
          ? { type: "xAxisRange", edge: "min", xmin: Math.min(x, interaction.payload.xmax - minimumSpan), xmax: interaction.payload.xmax }
          : { type: "xAxisRange", edge: "max", xmin: interaction.payload.xmin, xmax: Math.max(x, interaction.payload.xmin + minimumSpan) });
  ...
  const finishSvgInteraction = (event) => {
    ...
    } else if (dragPreview?.type === "xAxisRange") {
      history.set((current) => updateWorkspaceProject(current, activeMode, (currentWorkspace) => ({
        ...currentWorkspace,
  ...
  const renderPatternProperties = () => selectionCount > 1 ? (
    ...
    <>
      <Section title="Patron sélectionné">
        ...
        <Section title="Étiquette de courbe" defaultOpen={false}>
    ...
    <Section title="Note sélectionnée">
      <TextField label="Texte" value={String(activeNote.text ?? "Annotation")} onChange={(value) => updateNote(activeNote.id, "text", value)} />
      <div className="two-columns">
    ...
    <div className={`app-shell mode-${activeMode} density-${uiDensity} ${reduceMotion ? "reduce-motion" : ""} ${editorFullscreen ? "is-editor-fullscreen" : ""}`}>
      <header className="topbar masthead">
        <div className="masthead__edition">
          ...
          <span>Vol. 11 · Interaction & acquisition desk · Browser-local processing</span>
        ...
        <section key={activeMode} className="canvas-column">
          <div className="canvas-toolbar">
            ...
            <div className="canvas-toolbar__group">
              ...
              <IconButton icon="reset" title="Réinitialiser la plage X · double-clic sur la figure" onClick={resetDataZoom} />
            </div>
            <div className="canvas-toolbar__divider" />
            <div className="canvas-toolbar__group">
            ...
            <span className="canvas-toolbar__hint">Axe X : glisser la moitié gauche pour X min, la moitié droite pour X max · aperçu en direct · double-clic : réinitialiser · sélectionner une note puis utiliser ses poignées</span>
          </div>
          ...
          {showNavigator && visibleCount > 0 && (
            <RangeNavigator
              ...
              xmin={viewXMin}
              xmax={viewXMax}
              onCommit={(xmin, xmax) => history.set((current) => updateWorkspaceProject(current, activeMode, (currentWorkspace) => ({
                ...currentWorkspace,
                settings: { ...currentWorkspace.settings, xmin, xmax },
              })))}
            />
          )}
              ...
              <div className={`page-stage ${comparisonView ? "is-comparison" : ""}`} style={{ width: comparisonView ? W * displayZoom * 2 + 24 : W * displayZoom, height: H * displayZoom }}>
                ...
                <div className="figure-page" style={{ width: W * displayZoom, height: H * displayZoom }}>
                  <svg
                    ...
                    onClick={onSvgClick}
                    onDoubleClick={(event) => { event.preventDefault(); resetDataZoom(); }}
                  >
                    ...
                    {S.figureLayoutMode === "single" && S.mode === "raman" && zones.filter((zone) => zone.visible && Number(zone.xmax) > viewXMin && Number(zone.xmin) < viewXMax).map((zone) => {
                      ...
                      return (
                        <g key={`zone-${zone.id}`} opacity={selected ? 1 : 0.94} onClick={(event) => selectItem(event, "zone", zone.id)} style={{ cursor: "pointer" }}>
                      ...
                      return (
                        <g key={pattern.id} opacity={selectedByType.pattern.size && !isSelected("pattern", pattern.id) ? 0.72 : 1}>
                          ...
                          {(() => {
                            ...
                            return <g>
                              <text x={labelX - 12} y={labelYpx} dominantBaseline="middle" fontSize={Math.max(8, fontSize * 0.75)} fill={color} opacity="0.65" style={{ cursor: pattern.locked ? "not-allowed" : "ns-resize", userSelect: "none" }} onPointerDown={(event) => { if (!pattern.locked) beginCanvasDrag(event, "curveOrder", { id: pattern.id }); }}>↕</text>
                              <text
                                ...
                                onPointerDown={(event) => { if (!pattern.locked) beginCanvasDrag(event, "patternLabel", { id: pattern.id, dx, dy, fontSize }); }}
                              >{text}</text>
                    ...
                    {S.figureLayoutMode === "single" && S.showInset && (() => {
                      ...
                      const width = plotWidth * clamp(Number(S.insetWidthPct) || 34, 15, 70) / 100;
                      const height = mainHeight * clamp(Number(S.insetHeightPct) || 34, 15, 70) / 100;
                      const pad = 12;
                      const left = String(S.insetPosition).includes("left") ? M.left + pad : M.left + plotWidth - width - pad;
                      const top = String(S.insetPosition).includes("bottom") ? M.top + mainHeight - height - pad : M.top + pad;
                      const inner = { left: left + 30, right: left + width - 8, top: top + 18, bottom: top + height - 24 };
                      ...
                      const values = indices.map((entry) => insetPattern.displayY[entry.index]);
                      const min = Math.min(...values); const max = Math.max(...values); const range = max - min || 1;
                      const ix = (value) => inner.left + ((value - ixmin) / (ixmax - ixmin)) * (inner.right - inner.left);
                      ...
                      return <g><rect x={left} y={top} width={width} height={height} fill={S.pageBackground} stroke="#525b66" strokeWidth="0.8"/><line x1={inner.left} x2={inner.left} y1={inner.top} y2={inner.bottom} stroke="#20252b" strokeWidth="0.7"/><line x1={inner.left} x2={inner.right} y1={inner.bottom} y2={inner.bottom} stroke="#20252b" strokeWidth="0.7"/><path d={path} fill="none" stroke={color} strokeWidth={S.lineWidth} vectorEffect="non-scaling-stroke"/><text x={left + 7} y={top + 12} fontSize="8" fontWeight="700" fill="#20252b">{truncateLabel(insetPattern.label, 28)}</text><text x={(inner.left + inner.right) / 2} y={top + height - 6} textAnchor="middle" fontSize="7" fill="#343a40">{ixmin}–{ixmax}°</text></g>;
                    })()}
                    ...
                    {S.figureLayoutMode === "single" && hasAnnotations && annotationData.labels.map((tick, index) => {
                      ...
                      return <text
                        ...
                        onPointerDown={(event) => beginCanvasDrag(event, "phaseLabel", { id: tick.phaseId, xOffset: tick.labelOffsetX, yOffset: tick.labelOffsetY })}
                    ...
                    {S.figureLayoutMode === "single" && hasAnnotations && S.showAbbrevKey && phases.filter((phase) => phase.visible && phase.inAnnot).map((phase, index) => (
                      <text key={`key-${phase.id}`} x={xToPx(viewXMax) + 10} y={yToPx(annotationBase + S.tickScale * 0.84) + index * 14} fontSize="9" fontStyle="italic" fill={phase.color} fontFamily="Arial, Helvetica, sans-serif">{phase.abbrev} = {phase.name}</text>
                    ))}
                    ...
                    {S.figureLayoutMode === "single" && <><line x1={M.left} x2={M.left} y1={M.top} y2={M.top + mainHeight} stroke="#15191f" strokeWidth="1" /><text x="21" y={M.top + mainHeight / 2} fontSize={S.axisFontSize} fill="#15191f" textAnchor="middle" fontFamily="Arial, Helvetica, sans-serif" transform={`rotate(-90 21 ${M.top + mainHeight / 2})`}>{S.ylabel}</text></>}
                      ...
                      <g>
                        ...
                        {S.showPdfLegend && (() => {
                          ...
                          return <g>
                            ...
                            <rect data-ui-only="true" x={boxX} y={boxY} width={boxWidth} height="20" fill="#eef1f4" opacity="0.85" style={{ cursor: "move" }} onPointerDown={(event) => beginCanvasDrag(event, "phaseLegendMove", { x: boxX, y: boxY, width: boxWidth })} />
                    ...
                    {S.figureLayoutMode === "single" && (() => {
                      ...
                      return <g>
                        ...
                        <rect data-ui-only="true" x={M.left} y={axisY - 10} width={plotWidth / 2} height="36" fill="transparent" style={{ cursor: "ew-resize" }} onPointerDown={(event) => beginCanvasDrag(event, "xAxisRange", { edge: "min", xmin: S.xmin, xmax: S.xmax })} />
                        <rect data-ui-only="true" x={M.left + plotWidth / 2} y={axisY - 10} width={plotWidth / 2} height="36" fill="transparent" style={{ cursor: "ew-resize" }} onPointerDown={(event) => beginCanvasDrag(event, "xAxisRange", { edge: "max", xmin: S.xmin, xmax: S.xmax })} />
                        {dragPreview?.type === "xAxisRange" && <g data-ui-only="true" pointerEvents="none"><rect x={M.left} y={axisY - 28} width={plotWidth} height="18" fill="#ffffff" opacity="0.92"/><text x={M.left + 6} y={axisY - 15} fontSize="9" fill="#343a40">X min {previewMin.toFixed(S.mode === "drx" ? 3 : 1)}</text><text x={M.left + plotWidth - 6} y={axisY - 15} textAnchor="end" fontSize="9" fill="#343a40">X max {previewMax.toFixed(S.mode === "drx" ? 3 : 1)}</text></g>}
                      </g>;
                    })()}
          ...
          <div className="side-panel__content properties-scroll">
            {rightTab === "appearance" && (
              <>
                ...
                {activeMode === "drx" && <Section title="Axes DRX avancés" defaultOpen={false}>
                  <SelectField label="Axe X principal" value={S.xAxisMode || "2theta"} onChange={(value) => patchSettings("xAxisMode", value)} options={[["2theta", "2θ"], ["d", "d-spacing"], ["q", "Q"]]} />
                  ...
                  {S.showInset && <><SelectField label="Patron de l’encart" value={S.insetPatternId || ""} onChange={(value) => patchSettings("insetPatternId", value)} options={[["", "Patron sélectionné"], ...patterns.filter((pattern) => pattern.visible).map((pattern) => [pattern.id, pattern.label])]} /><div className="two-columns"><NumberField label="X min encart" value={S.insetXMin} step={0.5} onChange={(value) => patchSettings("insetXMin", value)} /><NumberField label="X max encart" value={S.insetXMax} step={0.5} onChange={(value) => patchSettings("insetXMax", value)} /></div><SliderField label="Largeur de l’encart" value={S.insetWidthPct} min={15} max={70} step={1} suffix="%" onChange={(value) => patchSettings("insetWidthPct", value)} /><SliderField label="Hauteur de l’encart" value={S.insetHeightPct} min={15} max={70} step={1} suffix="%" onChange={(value) => patchSettings("insetHeightPct", value)} /><SelectField label="Position" value={S.insetPosition} onChange={(value) => patchSettings("insetPosition", value)} options={[["top-right", "Haut droite"], ["top-left", "Haut gauche"], ["bottom-right", "Bas droite"], ["bottom-left", "Bas gauche"]]} /></>}
                </Section>}
            ...
            {rightTab === "processing" && (
              <>
                ...
                <Section title="Alignement par corrélation">
                  ...
                  <div className="inline-actions"><Button variant="primary" onClick={alignVisiblePatterns}>Aligner les patrons visibles</Button><Button variant="secondary" icon="reset" onClick={() => history.set((current) => updateWorkspaceProject(current, activeMode, (currentWorkspace) => ({ ...currentWorkspace, patterns: currentWorkspace.patterns.map((pattern) => pattern.locked ? pattern : ({ ...pattern, xoffset: (Number(pattern.xoffset) || 0) - (Number(pattern.alignmentShift) || 0), alignmentShift: 0, alignmentScore: undefined, alignmentReference: undefined })) })))}>Retirer l’alignement auto</Button></div>
