...
export default function App() {
  ...
  const axisPreviewActive = dragPreview?.type === "xAxisWindow"
  ...
  const saveSessionFile = useCallback(() => {
    const payload = JSON.stringify({ ...project, version: 15 }, null, 2);
                  ...
                  >
                    ...
                    {S.figureLayoutMode === "single" && S.showInset && (() => {
                      ...
                      const sourceRect = {
                        x: xToPx(ixmin),
                        y: yToPx(sourceYMax),
                        width: Math.max(2, xToPx(ixmax) - xToPx(ixmin)),
                      ...
                      return <g>
                        {S.insetShowSourceRect && <g data-ui-only={false}>
                        ...
                        <rect x={left} y={top} width={width} height={height} fill={S.pageBackground} stroke={collision ? "#e05a47" : "#525b66"} strokeWidth={collision ? 1.4 : 0.9}/>
                        <rect data-ui-only="true" x={left} y={top} width={width} height="19" fill="#eef1f4" opacity="0.86" style={{ cursor: canMove ? "move" : "default" }} onPointerDown={(event) => { if (canMove) beginCanvasDrag(event, "insetMove", { xFrac, yFrac, widthPct, heightPct }); }} />
                        ...
                        <text x={(inner.left + inner.right) / 2} y={top + height - 7} textAnchor="middle" fontSize="7" fill="#343a40">{ixmin}–{ixmax}°</text>
                    ...
                    {S.figureLayoutMode === "single" && (() => {
                      ...
                      return <g>
                        ...
                        <rect data-ui-only="true" x={M.left + 12} y={axisY - 8} width={Math.max(0, plotWidth - 24)} height="30" fill="transparent" style={{ cursor: dragPreview?.type === "xAxisWindow" && dragPreview.mode === "pan" ? "grabbing" : "grab" }} onPointerDown={(event) => beginCanvasDrag(event, "xAxisWindow", { mode: "pan", xmin: viewXMin, xmax: viewXMax, plotWidth })} />
                        <g data-ui-only="true">
                          <line x1={M.left} x2={M.left} y1={axisY - 12} y2={axisY + 12} stroke="#dc7848" strokeWidth="2.5" style={{ cursor: "ew-resize" }} onPointerDown={(event) => beginCanvasDrag(event, "xAxisWindow", { mode: "min", xmin: viewXMin, xmax: viewXMax, plotWidth })} />
                          <rect x={M.left - 7} y={axisY - 14} width="14" height="28" rx="3" fill="#dc7848" opacity="0.14" stroke="#dc7848" strokeWidth="0.8" style={{ cursor: "ew-resize" }} onPointerDown={(event) => beginCanvasDrag(event, "xAxisWindow", { mode: "min", xmin: viewXMin, xmax: viewXMax, plotWidth })} />
                          <line x1={M.left + plotWidth} x2={M.left + plotWidth} y1={axisY - 12} y2={axisY + 12} stroke="#dc7848" strokeWidth="2.5" style={{ cursor: "ew-resize" }} onPointerDown={(event) => beginCanvasDrag(event, "xAxisWindow", { mode: "max", xmin: viewXMin, xmax: viewXMax, plotWidth })} />
                          <rect x={M.left + plotWidth - 7} y={axisY - 14} width="14" height="28" rx="3" fill="#dc7848" opacity="0.14" stroke="#dc7848" strokeWidth="0.8" style={{ cursor: "ew-resize" }} onPointerDown={(event) => beginCanvasDrag(event, "xAxisWindow", { mode: "max", xmin: viewXMin, xmax: viewXMax, plotWidth })} />
                        </g>
                        {dragPreview?.type === "xAxisWindow" && <g data-ui-only="true" pointerEvents="none"><rect x={M.left} y={axisY - 31} width={plotWidth} height="19" fill="#ffffff" opacity="0.94"/><text x={M.left + 6} y={axisY - 17} fontSize="9" fill="#343a40">X min {previewMin.toFixed(S.mode === "drx" ? 3 : 1)}</text><text x={M.left + plotWidth / 2} y={axisY - 17} textAnchor="middle" fontSize="8.5" fill="#697482">{dragPreview.mode === "pan" ? "Déplacement de la fenêtre" : dragPreview.mode === "min" ? "Redimensionnement depuis X min" : "Redimensionnement depuis X max"}</text><text x={M.left + plotWidth - 6} y={axisY - 17} textAnchor="end" fontSize="9" fill="#343a40">X max {previewMax.toFixed(S.mode === "drx" ? 3 : 1)}</text></g>}
                      </g>;
                    })()}
