...
export default function App() {
  ...
  const saveSessionFile = useCallback(() => {
    const payload = JSON.stringify({ ...project, version: 7 }, null, 2);
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <div className="brand">
          <Logo />
          <div><strong>Make Figure</strong><span>DRX · Raman · v7</span></div>
        </div>
        ...
        <div className="mode-switch" aria-label="Mode d’analyse">
          {[["drx", "DRX"], ["raman", "Raman"]].map(([value, label]) => (
            <button type="button" key={value} className={activeMode === value ? "is-active" : ""} onClick={() => setMode(value)}>
              <span>{label}</span><small>{workspaceStats[value].total}</small>
            </button>
          ))}
        </div>
        <div className="topbar__divider" />
      ...
      <main className="workbench" style={{ gridTemplateColumns: `${leftWidth}px minmax(300px, 1fr) ${rightWidth}px` }}>
        <aside className="side-panel side-panel--left">
          ...
          <div className="side-panel__content">
            ...
            {leftTab === "phases" && (
              <>
                <button type="button" className="drop-button" onClick={() => phaseInputRef.current?.click()}><Icon name="upload" /><span><strong>Importer des phases</strong><small>{activeMode === "drx" ? ".dif ou liste de pics DRX" : "RRUFF ou liste de pics Raman"}</small></span></button>
        ...
        <section className="canvas-column">
          ...
          >
            {dropActive && <div className="drop-overlay"><Icon name="upload" size={30} /><strong>Déposer les fichiers</strong><span>.dif → DRX · RRUFF Raman → Raman · autres fichiers → espace actif.</span></div>}
            {!visibleCount ? (
